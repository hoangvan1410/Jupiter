using GAO.WebApi.Infrastructure.Identity;
...
{
    private readonly GAOTenantInfo _currentTenant;
    private readonly RoleManager<ApplicationRole> _roleManager;
    private readonly UserManager<ApplicationUser> _userManager;
    private readonly CustomSeederRunner _seederRunner;
    private readonly ILogger<ApplicationDbSeeder> _logger;

    public ApplicationDbSeeder(GAOTenantInfo currentTenant, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager, CustomSeederRunner seederRunner, ILogger<ApplicationDbSeeder> logger)
    {
    ...
    {
        foreach (string roleName in GAORoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == GAORoles.Basic)
            {
                await AssignPermissionsToRoleAsync(dbContext, GAOPermissions.Basic, role);
            }
            else if (roleName == GAORoles.Admin)
            {
                await AssignPermissionsToRoleAsync(dbContext, GAOPermissions.Admin, role);

                if (_currentTenant.Id == MultitenancyConstants.Root.Id)
                {
                    await AssignPermissionsToRoleAsync(dbContext, GAOPermissions.Root, role);
                }
            }
        }
    }

    private async Task AssignPermissionsToRoleAsync(ApplicationDbContext dbContext, IReadOnlyList<GAOPermission> permissions, ApplicationRole role)
    {
        var currentClaims = await _roleManager.GetClaimsAsync(role);
        foreach (var permission in permissions)
        {
            if (!currentClaims.Any(c => c.Type == GAOClaims.Permission && c.Value == permission.Name))
            {
                ...
                {
                    RoleId = role.Id,
                    ClaimType = GAOClaims.Permission,
    ...
    private async Task SeedAdminUserAsync()
    {
        ...
        {
            string adminUserName = $"{_currentTenant.Id.Trim()}.{GAORoles.Admin}".ToLowerInvariant();
            adminUser = new ApplicationUser
            {
                FirstName = _currentTenant.Id.Trim().ToLowerInvariant(),
                LastName = GAORoles.Admin,
        ...
        // Assign role to user
        if (!await _userManager.IsInRoleAsync(adminUser, GAORoles.Admin))
        {
            _logger.LogInformation("Assigning Admin Role to Admin User for '{tenantId}' Tenant.", _currentTenant.Id);
            await _userManager.AddToRoleAsync(adminUser, GAORoles.Admin);
        }
    }
}